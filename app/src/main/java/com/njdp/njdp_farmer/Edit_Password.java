...
public class Edit_Password extends AppCompatActivity {
    ...
    private String token, oldPassword, telephone;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(token == null || token.equals("") || oldPassword == null || oldPassword.equals("")){
            NormalUtil.error_hint(getApplicationContext(), "参数丢失，请重新登录后修改。");
            finish();
        }
                ...
                if (NormalUtil.isempty(text_oldpassword)) {
                    ...
                    if (NormalUtil.isempty(text_password2)) {
                        ...
                        if (text_password.getText().toString().equals(text_password2.getText().toString())) {
                            if (text_oldpassword.getText().toString().equals(oldPassword)) {
    ...
    public void showClick2(View v) {
        ...
        Edit_Password.this.text_password2.setTransformationMethod(PasswordTransformationMethod.getInstance());
    }
    public void revealClick2(View v) {
        password_reveal2.setVisibility(View.GONE);
        password_show2.setVisibility(View.VISIBLE);
        Edit_Password.this.text_password2.setTransformationMethod(HideReturnsTransformationMethod.getInstance());
    }
        ...
        if (!NetUtil.checkNet(Edit_Password.this)) {
            ...
            StringRequest strReq = new StringRequest(Request.Method.POST,
                ...
                public void onResponse(String response) {
                    ...
                    try {
                        ...
                        if (status == 0) {
                            ...
                            NormalUtil.error_hint(getApplicationContext(), "修改密码成功，请重新登录。");

                            // 重新登录
                            Intent i = getBaseContext().getPackageManager().getLaunchIntentForPackage(getBaseContext().getPackageName());
                            i.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
                            startActivity(i);
                            AgentApplication.cleanActivity();
                            finish();
                        } else if(status == 18){
                            // Error occurred in registration. Get the error
                            // message
                            NormalUtil.error_hint(getApplicationContext(), "请输入密码");
                        }else if(status == 26){
                            NormalUtil.error_hint(getApplicationContext(), "请输入原密码");
                        }
                        else if(status == 27){
                            NormalUtil.error_hint(getApplicationContext(), "原密码错误。");
                        }else {
                ...
                public void onErrorResponse(VolleyError error) {
                    ...
                    NormalUtil.error_hint(getApplicationContext(), error.getMessage());
                    hideDialog();
                }
            }) {
    ...
    private void editTextIsNull(){
        ...
        text_oldpassword.addTextChangedListener(new TextWatcher() {
            @Override
            ...
            public void afterTextChanged(Editable s) {
                if ((s.length() > 0) && !TextUtils.isEmpty(text_password.getText()) && !TextUtils.isEmpty(text_password2.getText())) {
        ...
        text_password.addTextChangedListener(new TextWatcher() {
            ...
            @Override
            public void afterTextChanged(Editable s) {
                if ((s.length() > 0) && !TextUtils.isEmpty(text_oldpassword.getText()) && !TextUtils.isEmpty(text_password2.getText())) {
                    finishEdit.setClickable(true);
                    finishEdit.setEnabled(true);
                } else {
                    finishEdit.setEnabled(false);
        ...
        text_password2.addTextChangedListener(new TextWatcher() {
            ...
            @Override
            public void afterTextChanged(Editable s) {
                if ((s.length() > 0) && !TextUtils.isEmpty(text_oldpassword.getText()) && !TextUtils.isEmpty(text_password.getText())) {
                    finishEdit.setClickable(true);
                    finishEdit.setEnabled(true);
                } else {
                    finishEdit.setEnabled(false);
