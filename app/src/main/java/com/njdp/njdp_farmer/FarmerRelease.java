...
import com.njdp.njdp_farmer.MyClass.PictureCompress;
...
public class FarmerRelease extends AppCompatActivity {
    ...
    private static final int GETADDRESSPIC = 3;
    ...
    private String token, strAddress;
    private ProgressDialog pDialog;
    private List<String> imageList;         //街景图片的存储路径
    private ArrayList<String> PicsArray;    //街景图片服务器返回的路径
    ...
    private void initView() {
        ...
        if(isEdit){
            ...
            strAddress = farmlandInfo.getProvince() + "-" + farmlandInfo.getCity() + "-" + farmlandInfo.getCounty() + "-" +
                    farmlandInfo.getTown() + "-" + farmlandInfo.getVillage();
            String add;
            //判断地址长度，防止被右边的天气图标遮挡文字
            if(strAddress.length() > 30){
                add = strAddress.substring(0, 28);
                add+="…";
            }
            else{
                add = strAddress;
            ...
            if(add.length() > 15){
                String a = add.substring(0,14);
                String b = add.substring(14);
                add = a + "\n" + b;
            }
            address.setText(add);
            starttime.setText(farmlandInfo.getStart_time_String());
            endtime.setText(farmlandInfo.getEnd_time_String());
            if(farmlandInfo.getStreet_view().equals("null") || farmlandInfo.getStreet_view().length()==0){
                addresspic.setText("");
            }else {
                Collections.addAll(PicsArray, farmlandInfo.getStreet_view().split(";"));
                addresspic.setText("上传了" + PicsArray.size() + "张图片");
            }
        ...
        public void onClick (View v) {
            ...
            switch (v.getId()) {
                ...
                case R.id.addresspic:
                    Intent intent2 = new Intent(FarmerRelease.this, AddressPicUp.class);
                    intent2.putExtra("isEdit", isEdit);
                    intent2.putExtra("token", token);
                    intent2.putExtra("id", String.valueOf(farmlandInfo.getId()));
                    intent2.putStringArrayListExtra("pics", PicsArray);
                    startActivityForResult(intent2, GETADDRESSPIC);
                    break;
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        switch (requestCode) {
            case ADDRESSEDIT:
                ...
                break;
            case GETADDRESSPIC:
                imageList = data.getStringArrayListExtra("imagList");
                addresspic.setText("选择了" + imageList.size() + "张图片上传");
                break;
        }
    }
    ...
    public void weatherClick(View view) {
        if(farmlandInfo.getCity().equals("") || farmlandInfo.getCounty().equals("")){
            Toast.makeText(getApplicationContext(), "还没有选择发布地址。", Toast.LENGTH_SHORT).show();
            return;
        }
        Intent intent = new Intent(FarmerRelease.this, WindowWeather.class);
        intent.putExtra("city", farmlandInfo.getCity());
        intent.putExtra("country", farmlandInfo.getCounty());
    ...
    //上传图片
    int picIndex;
    private void uploadImage(final String temppath, final String picName) {

        //final File fileNew=new File(temppath);
        if (!AgentApplication.permission_WRITE_EXTERNAL_STORAGE || !AgentApplication.permission_READ_EXTERNAL_STORAGE) {
            error_hint("读取图片失败，请在权限设置管理界面开放存储卡读写权限。");
            return;
        }
        final File fileNew = PictureCompress.getSmallImageFile(temppath);
        if(fileNew == null){
            Toast.makeText(getApplicationContext(), "压缩图片错误：创建文件失败！", Toast.LENGTH_SHORT).show();
            return;
        }
        String editTag = "0";
        if(isEdit && picName.equals("addressPic1")){
            editTag = "1"; //1为更新街景图片，服务器会先清空旧的街景图片
        }

        pDialog.setMessage("正在上传街景图片" + (picIndex+1) + " ...");
        showDialog();

        if (!fileNew.exists()) {
            hideDialog();
            Toast.makeText(getApplicationContext(), "街景图片不存在!请重新选择！", Toast.LENGTH_SHORT).show();
        } else if (!NetUtil.checkNet(FarmerRelease.this)) {
            hideDialog();
            error_hint("网络连接错误");
        } else {
            try {
                OkHttpUtils.post()
                        .url(AppConfig.URL_UPADDRESSPIC)
                        .addParams("token", token)
                        .addParams("id", id)
                        .addParams("editTag",editTag)
                        .addFile("Farmlands_street_view", picName, fileNew)
                            ...
                            public void onResponse(String response) {
                                ...
                                try {
                                    ...
                                    if (status == 0) {
                                        ...
                                        if(picIndex < imageList.size()){
                                            uploadImage(imageList.get(picIndex), "addressPic" + (picIndex + 1));
                                        }else {
                                            error_hint("街景图片上传成功！");
                                            setContentNUll();
                                        }
                                    ...
                                    Log.e("AddressPicUp", "2 Json error：response错误： " + e.getMessage());
                                }
                            }
                        });
            }
        ...
        public void onResponse(String response) {
            ...
            try {
                ...
                if (status == 0) {
                    ...
                    if(isEdit) {
                        error_hint("农田信息修改成功！");
                        id = String.valueOf(farmlandInfo.getId());
                    }
                    else {
                        error_hint("发布成功！");
                        id = jObj.getString("result");
                    }

                    if(imageList.size() > 0) {
                        picIndex = 0;
                        uploadImage(imageList.get(0), "addressPic1" );
                    }else {
                        //clean frament
                        setContentNUll();
                    }
    ...
    private Location getLocalGPS() {
        ...
        //获取并返回Location
        if (ActivityCompat.checkSelfPermission(FarmerRelease.this, Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED
    ...
    private void editTextIsNull(){
        ...
        address.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                ...
                if (temp.length > 4) {
                    ...
                    mSearch.geocode(new GeoCodeOption().city(farmlandInfo.getCity()).address(farmlandInfo.getCounty() + " " + farmlandInfo.getVillage()));
                }
            }
        });
        ...
        addresspic.addTextChangedListener(new TextWatcher() {
            @Override
            public void beforeTextChanged(CharSequence s, int start, int count, int after) {
            ...
            @Override
            public void afterTextChanged(Editable s) {
                if (!TextUtils.isEmpty(area.getText()) && !TextUtils.isEmpty(price.getText()) && !TextUtils.isEmpty(croptype.getText())
                        && !TextUtils.isEmpty(address.getText()) && !TextUtils.isEmpty(starttime.getText()) && !TextUtils.isEmpty(endtime.getText())) {
                    releaseEditFinish.setClickable(true);
                    releaseEditFinish.setEnabled(true);
                } else {
        ...
        public void onGetGeoCodeResult(GeoCodeResult geoCodeResult) {
            if (geoCodeResult == null || geoCodeResult.error != SearchResult.ERRORNO.NO_ERROR) {
                ...
                if(firstSearchBaiduGPS){
                    ...
                    mSearch.geocode(new GeoCodeOption().city(farmlandInfo.getCity()).address(farmlandInfo.getCounty() + " " + farmlandInfo.getVillage().substring(0, farmlandInfo.getVillage().length()-1)));
                    return;
                }
                //未找到村庄位置，显示提示信息
