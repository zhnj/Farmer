...
public class getpassword extends AppCompatActivity {
    ...
    public void getPassword_next(View v) {
        String tag_string_req = "req_farmer_VerifyCodeNext";
        if(!NetUtil.checkNet(getpassword.this)){
            NormalUtil.error_hint(getApplicationContext(), "网络连接错误");
        } else {
            StringRequest strReq = new StringRequest(Request.Method.POST,
                    AppConfig.URL_GETPASSWORD21, nextSuccessListener, mErrorListener) {
                @Override
                protected Map<String, String> getParams() {

                    Map<String, String> params = new HashMap<>();
                    //params.put("name", name);
                    params.put("phone", text_telephone.getText().toString().trim());
                    params.put("code", text_VerificationCcode.getText().toString().trim());
                    return params;
                }
            };
    ...
    private Response.Listener<String> nextSuccessListener =new Response.Listener<String>() {
        ...
        public void onResponse(String response) {
            ...
            try {
                ...
                if (status == 0) {
                    ...
                    intent.putExtra("telephone", text_telephone.getText().toString().trim());
