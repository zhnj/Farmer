...
import com.njdp.njdp_farmer.UAVListActivity;
...
public class UAVMachineSearch extends Fragment implements View.OnClickListener {
    ...
    private int Search_range=5;                               //查询无人机的距离
    ...
    private ArrayList<MachineInfo> machineInfos;            //查询回来的无人机
    ...
    private UAVMachineSearch.mListener myListener = new UAVMachineSearch.mListener();
    /**
     * 当前定位的模式
     */
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        try {
            ...
            //添加覆盖物鼠标点击事件
            mBaiduMap.setOnMarkerClickListener(new UAVMachineSearch.markerClicklistener());
            /////////////////地图代码结束////////////////////////
            ...
            runnable = new Runnable(){
                ...
                public void run() {
                    ...
                    try {
                        ...
                        //获取无人机数据
                        if(farmlandInfo != null) //如果传递过来的参数为空，则在mListener地图定位后，使用当前位置搜索无人机
                        {
                            // 构造定位数据
                            ...
                            Log.e("无人机查询------------->", "使用农田位置查询无人机");
                        }
                        else {
                            isUseLocalGPS = true;
                            Log.e("无人机查询------------->", "没有找到农田信息，使用本地位置查询无人机");
                        }
                        getMachineInfos();
                    } catch (Exception e) {
                        e.printStackTrace();
    ...
    public View inFlater(LayoutInflater inflater) {
        view = inflater.inflate(R.layout.activity_uav_machine_search, null, false);
        initView(view);
        // 获取屏幕的高度和宽度
...
//                    //更新无人机数据
//                    machineListView.setText("共有" + machinesToShow.size()  + "条无人机信息，点击查看列表");
//                    ShowInMap(machinesToShow);
//                }else {
//                    machineListView.setText("共有0条无人机信息，点击查看列表");
//                }
                break;
...
//                }
            ...
            case R.id.machineListView:
                Log.e("------------->", "点击查看无人机列表");
                Intent intent = new Intent(getActivity(), UAVListActivity.class);
...
//            LatLng desLatLng = converter.convert();
        ...
        public boolean onMarkerClick(Marker marker) {
            ...
            if(phoneBtn != null) {
                phoneBtn.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        intentCallPhone = new Intent(Intent.ACTION_CALL, Uri.parse("tel:" + telephone));
                        intentCallPhone.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                        //确认拨打电话的权限
    ...
    private PopupWindow makePopupWindow(Context cx, final MachineInfo machineInfo)
    {
        ...
        driver_name.setText("飞服公司名称：" + machineInfo.getName());
        driver_phone.setText("公司电话：" + machineInfo.getTelephone());
        range.setText("距离：" + machineInfo.getRange() + "km");
        qq.setText(" 公司地址 ：" + machineInfo.getQq());
        weixin.setText("员工个数：" + machineInfo.getWeixin());
        work_time.setText("负责人：" + machineInfo.getWork_time() );
        remark.setText("公司介绍：" + machineInfo.getRemark());
    ...
    public void getMachineInfos() {
        ...
        String tag_string_req = "req_uav_machines_get";
        final FarmlandInfo searchFarmland;

        if(isFirst) {
            pDialog.setMessage("正在获取无人机数据 ...");
            showDialog();
            //isFirst = false;
        }
        //农田信息为空则返回
        ...
        if (!NetUtil.checkNet(getActivity())) {
            ...
            StringRequest strReq = new StringRequest(Request.Method.POST,
                    AppConfig.URL_UAV_SEARCH, mSuccessListener, mErrorListener) {
            ...
            try {
                ...
                if (status == 0) {
                    ...
                    for(int i = 0; i < jObjs.length(); i++){
                        ...
                        temp.setLatitude(object.getDouble("com_latitude"));
                        temp.setLongitude(object.getDouble("com_longitude"));
                        temp.setName(object.getString("com_name"));
                        temp.setTelephone(object.getString("com_tel"));
                        temp.setQq(object.getString("com_addr")); //公司地址
                        temp.setWeixin(object.getString("staff_num"));//员工数量
                        ...
                        temp.setWork_time(object.getString("com_charge"));//费用信息
                        temp.setRemark(object.getString("com_memo"));
...
//                    }
                    ...
                    machineListView.setText("共有" + AgentApplication.machinesToShow.size()  + "条无人机信息，点击查看列表");
                } else if(status == 3){
                    //密匙失效
    ...
    public void onDestroy() {
        ...
        mBaiduMap.clear();
        mBaiduMap.removeMarkerClickListener(new UAVMachineSearch.markerClicklistener());
